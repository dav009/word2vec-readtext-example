...
public class Word2VecTextReader
{
    ...
    protected JavaSparkContext connectSpark(int minWords, int vectorLength){
    ...
    protected Pair<VocabCache,WeightLookupTable> vectorizeWords(String fileName, JavaSparkContext sc) throws Exception {
        JavaRDD<String> rdd = sc.textFile(new File(fileName).toURI().toString());
        Word2Vec vec = new Word2Vec();
        return vec.train(rdd);
    }

    protected void saveVectors( Pair result, String outputFileName, Boolean serialize ) throws Exception {
        if (serialize) {
